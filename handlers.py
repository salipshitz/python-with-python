...
class SqlHandler:
    ...
    def login(self, username, password):
        cmd = "SELECT * FROM users WHERE ((username = {0} OR email = {0}) AND password = {1});".format(username, password)
        self.cursor.execute(cmd)
    ...
    def signup(self, username, email, password):
        cmd = "SELECT * FROM users WHERE (username = \"{0}\" OR email = \"{1}\");".format(username, email)
        self.cursor.execute(cmd)
        try:
            assert user == None
        except AssertionError:
            ...
            return False
        cmd = "INSERT INTO users VALUES(NULL, {0}, {1}, {2});".format(username, email, password)
